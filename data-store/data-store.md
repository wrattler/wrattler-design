...
The second kind of symbol, perhaps more interesting to us, are the ones that relate
...
As another example, suppose $C$ is a concept and $R$ is a role (*i.e.*, a
two-place relation). Then the predicate whose first-order logic form is
written:
$$
Q(x) = (\exists y)(R(x, y) \wedge C(x))
$$
...
Finally, to encode the assumption above, that “every person has an age that is a
